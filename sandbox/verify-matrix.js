...
check('Opportunité massive', { ease: 9, impact: 9, urgency: 4 }, 'Opportunité massive');
check('Corvée express', { ease: 1, impact: 0, urgency: 4 }, 'Corvée express');
check('Victoire rapide', { ease: 9, impact: 7, urgency: 1 }, 'Victoire rapide');
check('Chemin critique', { ease: 1, impact: 9, urgency: 4 }, 'Chemin critique');

console.log('\n=== New matrix rules ===');
check('Accélérateur', { ease: 5, impact: 9, urgency: 4 }, 'Accélérateur');
check('Coup de pouce', { ease: 9, impact: 5, urgency: 4 }, 'Coup de pouce');
check('À arbitrer', { ease: 5, impact: 5, urgency: 2 }, 'À arbitrer');
check('Piste exploratoire', { ease: 5, impact: 5, urgency: 1 }, 'Piste exploratoire');
check('Pression modérée', { ease: 5, impact: 5, urgency: 4 }, 'Pression modérée');
check('Échéance serrée', { ease: 3, impact: 5, urgency: 4 }, 'Échéance serrée');

var sparseCtx = { tier: tier, rules: PriorityMatrix.RULES.filter(function (r) { return r.id === 'backlog-filler'; }) };
var fallback = PriorityMatrix.resolveLabel({ ease: 5, impact: 5, urgency: 2 }, sparseCtx);
...
var disabledResult = PriorityMatrix.resolveLabel({ ease: 9, impact: 9, urgency: 4 }, disabledCtx);
...
var customResult = PriorityMatrix.resolveLabel({ ease: 9, impact: 9, urgency: 4 }, customCtx);
...
check('Victoire rapide alias', { ease: 9, impact: 7, urgency: 1 }, 'Victoire rapide', aliasCtx);
