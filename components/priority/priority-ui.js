...
(function (global) {
  ...
  /** Start date → Trello `start` ISO (optional startTime; else local midnight). */
  function inputsToTrelloStartIso(inputs) {
    if (!inputs) return '';
    var startDate = normalizeDueDate(inputs.startDate);
    if (!startDate) return '';
    return trelloDueIsoFromParts(startDate, inputs.startTime || null) || '';
  }
    ...
    function renderMembersPicker() {
      ...
      if (!available.length) {
        if (!boardMembers.length) {
          ...
          empty.textContent = 'Aucun membre Trello sur ce tableau';
  ...
  var PriorityUI = {
    ...
    setCustomAssigneeCatalog: setCustomAssigneeCatalog,
    findCustomAssigneeCatalogById: findCustomAssigneeCatalogById,
    findCustomAssigneeCatalogByName: findCustomAssigneeCatalogByName,
    upsertCustomAssigneeCatalog: upsertCustomAssigneeCatalog,
